...
def main(file, K = 1, R = 1, P = 0, H = 2):
    TIME_S = monotonic()
    [nodes, RESULT, MAC_TIME, COUNT, COUNT_H, COUNT_C] = MAC(H, READ_FILE(f"dom{file}.txt", f"var{file}.txt", f"ctr{file}.txt"), Solution = deque(), r = R, probes = P, function = heuristics[H], k = K)
    TIME_F = monotonic()
    TIME = TIME_F - TIME_S
...
for file in ["2-f24", "2-f25", "3-f10", "3-f11", "11", "6-w2", "7-w1-f4", "7-w1-f5", "8-f10", "8-f11", "14-f27", "14-f28"] :
